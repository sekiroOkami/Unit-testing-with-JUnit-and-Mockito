...
public class WikiUtil {
    public static String getWikipediaExtract(String title) {
        ...
        String queryString = params.entrySet().stream().map(Map.Entry::toString).collect(Collectors.joining("&"));
        try {
    ...
    private static String getResponse(String url) throws IOException, InterruptedException {
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create(url)).timeout(Duration.ofSeconds(2)).build();
